...
for (const f of fixtures) {
  ...
  results.push({
    f, tp, fn, ghost: ghosts, recall, precision, noiseRejection, staleOk,
...
console.log("fixture            recall  prec   noise%  stale  files(GT/rep)");
for (const r of rows) {
  ...
  console.log(`${name} ${recall}  ${prec}   ${noise}   ${stale}  ${files}${ghost}`);
}
console.log("\nmacro-averages:");
...
const failed = results.filter(r => r.error).length + rows.filter(r => r.recall < 1 || r.noiseRejection < 1 || r.staleOk < 1).length;
console.log(`\n${failed === 0 ? "PASS" : "FAIL"} — ${rows.filter(r => r.recall >= 1 && r.noiseRejection >= 1 && r.staleOk >= 1).length}/${rows.length} fixtures fully correct`);
